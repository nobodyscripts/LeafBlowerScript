...
Class TheExaltedBridge extends Zone {
    ...
    AttemptTravel(delay, scrolldelay := 0, extradelay := 0) {
        ...
        If (TheExaltedBridgeLeaf) {
            ...
            Out.I("The Exalted Bridge leaf not found while trying to travel.")
        }
        Sleep(delay + extradelay)
    ...
    ClickTravelButton(coord, delay) {
        ...
        ;Button := Points.Areas.SacredNebula.TheExaltedBridge
        Button := cLBRButton()
        Out.D("Zone travel button colour " Button.GetColour())
        ; If no button we are misaligned
        If (!Button.ClickButtonActive(, , delay, S.Get("NavigateTime") + delay)) {
            Out.I("The Exalted Bridge travel: Button not found.")
