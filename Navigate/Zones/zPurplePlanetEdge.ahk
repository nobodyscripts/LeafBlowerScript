...
Class PurplePlanetEdge extends Zone {
    ...
    AttemptTravel(delay, scrolldelay := 0, extradelay := 0) {
        ...
        If (PurplePlanetEdgeLeaf) {
            ...
            Log("Purple Planet Edge leaf not found while trying to travel.")
        }
        Sleep(delay + extradelay)
    ...
    ClickTravelButton(coord, delay) {
        ...
        ;Button := Points.Areas.EnergyB.PurplePlanetEdge
        Button := cPoint()
        DebugLog("Zone travel button colour " Button.GetColour())
        ; If no button we are misaligned
        If (!Button.ClickButtonActive(, , delay, NavigateTime + delay)) {
            Log("Purple Planet Edge travel: Button not found.")
