...
Button_Click_GameHotkeys(*) {
    ...
    optionsGUI.Add("Button", "default xs", "Save").OnEvent("Click", SaveGameHotkeysInput)
    optionsGUI.Add("Button", "default yp", "Cancel").OnEvent("Click", CloseGameHotkeys)
    optionsGUI.Add("Button", "default yp", "Reset To Defaults").OnEvent("Click", ResetGameHotKeys)
    optionsGUI.Add("Button", "default yp", "Apply To Game").OnEvent("Click", ApplyNewHotkeysToGame)
    ...
    ApplyNewHotkeysToGame(*) {
        if (WinExist(LBRWindowTitle)) {
            MsgBox("CLOSE GAME BEFORE RUNNING APPLY SCRIPT`r`nChanges not saved.")
            return
        }
